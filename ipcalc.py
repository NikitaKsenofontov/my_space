# -*- coding: utf8 -*-
from my_space import general_input as g_i
...
def not32(int32):
    ...
    return int32 ^ 0xffffffff if int32 is not None else None
...
def ip_str_to_int(net_ip):
    ...
    if len(arr_ip) != 4:
        return None
    octs = [str_to_int(arr_ip[0], 0, 255), str_to_int(arr_ip[1], 0, 255), str_to_int(arr_ip[2], 0, 255),
            str_to_int(arr_ip[3], 0, 255)]
...
def ip_int_to_str(ip):
    return (str(ip >> 24) + '.' + str((ip >> 16) & 0xFF) + '.'
            + str((ip >> 8) & 0xFF) + '.' + str(ip & 0xFF)) if ip is not None else None
...
def wildcard_mask(mask):
    return not32(mask) if mask is not None else None


def pref_to_mask(prefix):
    return (2 ** prefix - 1) << (32 - prefix) if prefix is not None else None


def is_mask_invalid(mask):
    # check non-prefix mask format (1...1 at left, 0...0 at right)
    inverted_mask = not32(mask)
    return inverted_mask & (inverted_mask + 1) != 0 if inverted_mask is not None else False
...
def net_addr(ip, net_mask):
    return ip & net_mask if (ip is not None) and (net_mask is not None) else None


def broadcast(ip, net_mask):
    return ip | wildcard_mask(net_mask) if (ip is not None) and (net_mask is not None) else None
...
def subnets_allocation(subnet_list, subnets_number, ip, net_mask):
    ...
    for i in range(1, subnets_number, 1):
        ...
        if base_ip > prev_ip:
            ...
            subnet_list[i][SUBNET_IP] = ((prev_ip >> (32 - pref_by_mask(subnet_list[i][SUBNET_MASK]))) + 1) << (
                    32 - pref_by_mask(subnet_list[i][SUBNET_MASK]))
...
def main():
    ...
    subnets = g_i.universal_input("Enter number of subnets \n", "Invalid number of subnets. " +
                                  "Number of subnets with at least 1 host "
                                  "must be specified by integer value in the range of 1 to " + str(max_subnets),
                                  lambda st: str_to_int(st, 1,
                                                        max_subnets))
    # ограничиваем число подсетей, которое можно создать: минимум 1, максимум max_subnets
    ...
    for i in range(subnets):
        subnet_list.append([i + 1, g_i.universal_input("Enter number of devices in subnet " + str(i + 1) + "\n",
                                                       ...
                                                       "Number of hosts "
                                                       "must be specified by integer value in the range of 1 to "
                                                       + str(max_sub_hosts),
    ...
    for i in range(subnets):
        print("\nFor subnet %s, " % subnet_list[i][SUBNET_NUMBER] + "subnet ip is %s, " % ip_int_to_str(
            subnet_list[i][SUBNET_IP])
